...
**When testing file logging** (activity.log, error.log), pass `PLANDERSON_BASE_DIR` via `useTempDir()` to redirect log output to a known, isolated location. `spawnHook` translates this to a `dev.json` file in the subprocess's fake HOME:
